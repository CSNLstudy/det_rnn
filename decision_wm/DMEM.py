...
tf.saved_model.save(model, "/Volumes/Data_CSNL/project/RNN_study/20-10-15/HG/output/decision/success10")
...
for i in range(20):
    ...
    for iter in range(6000):
        ...
        if iter < 4000:
            ...
            dt.hp['lam_estim'] = 300.; dt.hp['lam_decision'] = 2400.
    model.model_performance = dt.tensorize_model_performance(model_performance)
    tf.saved_model.save(model, "/Volumes/Data_CSNL/project/RNN_study/20-10-15/HG/output/decision/success"+str(i))
...
## Test the robustness of DIV patterns
...
for i in range(20):
    ...
    for i_s, s_t in enumerate(stim_list):
        for i_r, ref in enumerate(ref_list):
            behav_iqrmat[i,i_s,i_r] = scipy.stats.iqr(behav_dict[str(i)].corrError[(behav_dict[str(i)].reference_ori_total == ref) & (behav_dict[str(i)].stimulus_ori_total == s_t)])
...
for i in range(20):
    plt.plot(np.mean(behav_iqrmat[i,:,:],axis=0))
plt.show()

for i in range(20):
    plt.scatter(np.mean(behav_iqrmat[i,:,[3,5]]),np.mean(behav_iqrmat[i,:,[0,8]]))
...
for ref in ref_list:
    plt.plot(np.arange(750), np.mean(H_total[:, H_ref == ref, 80:], axis=(1,2)), label=str(ref))
plt.legend()
plt.show()
...
decode_dict    = {}
decode_perfmat = np.ones((20,75,9)) * np.nan
for i in range(20):
    with open('/Volumes/Data_CSNL/project/RNN_study/20-10-15/HG/output/decoded/sequential' + str(i) + '.pkl', 'rb') as f:
        decode_dict[str(i)] = pickle.load(f)
    for i_r in range(9):
        for i_t in range(75):
            decode_perfmat[i, i_t, i_r] = corrcoef(np.repeat(np.arange(24), 10) / 24. * np.pi,
...
##
for i in range(20):
    plt.plot(np.mean(decode_perfmat[i,:],axis=1))
plt.legend()
plt.show()

for i_r in range(9):
    plt.plot(np.mean(decode_perfmat[:,:,i_r],axis=0), label=str(ref_list[i_r]))
plt.legend()
plt.ylim([0.95,1.0])
plt.show()
...
plt.imshow((model.var_dict['w_rnn'].numpy() * dt.hp['EI_mask'].numpy()).T, cmap='seismic', vmax=11., vmin=-11.)
plt.colorbar()
plt.show()
