...
if not os.path.isdir(savedir + '/estimation/Iter' + str(iteration)):
    os.makedirs(savedir + '/estimation/Iter' + str(iteration))
...
def run_model(in_data, var_dict, syn_x_init, syn_u_init):
    ...
    syn_x = syn_x_init
    syn_u = syn_u_init
    w_rnn = par['EI_mask'] @ np.maximum(var_dict['w_rnn'], 0)
...
a1 = axes[1].imshow(ntarget[:,TEST_TRIAL,:].T, aspect='auto', vmin=0, vmax=0.15); axes[1].set_title("Desired Output"); fig.colorbar(a1, ax=axes[1])
a2 = axes[2].imshow(cenoutput[:,TEST_TRIAL,:].T, aspect='auto', vmin=0, vmax=0.15); axes[2].set_title("Predicted"); fig.colorbar(a2, ax=axes[2])
...
##
par['design'].update({'iti'     : (0, 5.5),
                      'stim'    : ((5.5,7.0),(20.5,22.0)),
                      'delay'   : ((7.0,12.5),(17.0,23.5),(22.0,23.5)),
                      'estim'   : ((12.5,17.0),(23.5,28.0))})
par['output_range'] = ((12.5,17.0),(23.5,28.0))
par['dead'] = ((0,0.1),(23.5,23.6))
par['input_rule'] = {'fixation' : (0,28.0),
                     'response' : ((12.5,17.0),(23.5,28.0))}
par['output_rule'] = {'fixation' : ((0,12.5),(17.0,23.5))}
par = update_parameters(par)
stimulus = Stimulus(par)
trial_info = stimulus.generate_trial()
...
a0 = axes[0].imshow(trial_info['neural_input'][:,TEST_TRIAL,:].T, aspect='auto'); axes[0].set_title("Neural Input"); fig.colorbar(a0, ax=axes[0])
# a1 = axes[1].imshow(ntarget[:,TEST_TRIAL,:].T, aspect='auto', vmin=0, vmax=0.15); axes[1].set_title("Desired Output"); fig.colorbar(a1, ax=axes[1])
a1 = axes[1].imshow(cenoutput[:,TEST_TRIAL,:].T, aspect='auto', vmin=0, vmax=0.15); axes[1].set_title("Predicted"); fig.colorbar(a1, ax=axes[1])
fig.tight_layout(pad=2.0)
plt.show()
