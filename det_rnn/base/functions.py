...
wrr_vec = np.array([ 0.33191967,  0.31758046,  0.27536497,  0.20851054,  0.12301451,
        0.02620409, -0.07430199, -0.17120331, -0.25855514, -0.33270113,
       -0.3905031 , -0.42825056, -0.44136119, -0.42825056, -0.3905031 ,
       -0.33270113, -0.25855514, -0.17120331, -0.07430199,  0.02620409,
        0.12301451,  0.20851054,  0.27536497,  0.31758046])
...
def w_design(w_name, par):
    if w_name == 'w_in1':
        ...
        dexp      = win1_vec
        quo, rem  = divmod(par['n_hiddenr'], n)
        ...
        for i in range(24):
            w[:,i]    = np.roll(w21_vec[:par['n_hidden2']],  6+i)
            w[:,i+24] = np.roll(w21_vec[:par['n_hidden2']], -6+i)

    elif w_name   == 'w_rnn22':
        # TODO(HG): generalize here
        w = np.zeros((par['n_hidden2'],par['n_hidden2']))
        for i in range(par['n_hidden2']):
            w[:, i] = np.roll(w22_vec[:par['n_hidden2']], i)
    
    elif w_name  == 'w_rnnrr':
        w = np.zeros((par['n_hiddenr'],par['n_hiddenr']))
        for i in range(par['n_hiddenr']):
            w[i, :] = np.roll(wrr_vec[:par['n_hidden2']], i)
