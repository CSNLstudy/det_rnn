...
par = {
	...
	'design':  {'iti'	: (0, 1.5),
			   	'stim'	: (1.5, 3.0),
			   	'delay'	: (3.0, 3.2),
				'crit' : (3.2, 3.7), # Eva added
				'dec': (3.7, 4.5), # Eva added
			   	'estim'	: (4.5, 6.0)},
	# 'output_range' : ((4.5,6.0)), # None fills the whole period
	'output_range' : ((4.5,6.0)), # Estimation output
	'Doutput_range' : ((3.7,4.5)), # Decision output added
	# Mask specs
	# 'dead': ((0,0.1),(4.4,4.5)),
	# 'mask': {'iti'	: 1., 'stim' : 1., 'delay'	: 1., 'estim' : 5.,
	# 		 'rule_iti' : 2., 'rule_stim' : 2., 'rule_delay' : 2., 'rule_estim' : 10.},  # strength
	'mask': {'iti'	: 1., 'stim' : 1., 'delay'	: 1.,'dec': 0., 'estim' : 5.,
			 'rule_iti' : 2., 'rule_stim' : 2., 'rule_delay' : 2., 'rule_dec': 10., 'rule_estim' : 10.},  # strength
	'dm_mask': {'iti'	: 1., 'stim' : 1., 'delay'	: 1.,'dec': 5., 'estim' : 0.,
			 'rule_iti' : 2., 'rule_stim' : 2., 'rule_delay' : 2., 'rule_dec': 10., 'rule_estim' : 10.},

	'dead': ((0,0.1),(3.7,3.8)), # Eva added
	...
	'loss_fun'	: 'sse', # 'cosine', 'mse', 'mse_normalize', 'centropy'
}
...
def update_parameters(par):
	...
	par.update({'design_rg': _convert_to_rg(par['design'], par['dt']),
				...
				# Eva added
				'Doutput_rg': _convert_to_rg(par['Doutput_range'], par['dt']),
				#########
