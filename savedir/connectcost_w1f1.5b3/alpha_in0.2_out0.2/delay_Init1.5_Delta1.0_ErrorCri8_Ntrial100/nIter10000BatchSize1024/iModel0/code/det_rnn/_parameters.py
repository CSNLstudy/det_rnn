...
par = {
	...
	'modular'				: False,
	'connect_cost_within'	: 0.8,
	'connect_cost_forward'	: 0.4,
	'connect_cost_back'		: 0.2,
	...
	'n_untuned_input'	: 100,		 # number of rnn units TODO(HL): h_hidden to 100
...
def update_parameters(par):
	...
	## set n_input
	par['n_input'] = par['n_rule_input'] + par['n_tuned_input'] + par['n_untuned_input']
	...
	## set n_total
	par['n_total'] = par['n_input'] + par['n_output']

	#
	par['EI_mask'] = _EI_mask(par['n_total'], par['exc_inh_prop'])
	par['_modular_weight_cost_mask'] = _modular_weight_cost_mask(par['n_input'], par['n_output'],
											   par['connect_cost_within'],
											   par['connect_cost_forward'],
											   par['connect_cost_back'])

	par['tuned_silencing_mask'], par['untued_silencing_mask'], par['out_silencing_mask'] \
						= _silencing_mask(par['n_tuned_input'], par['n_untuned_input'], par['n_output'])

	par['alpha_mask'] = _alpha_mask(par['n_input'], par['n_output'],
									par['alpha_input'], par['alpha_output'],
									par['batch_size'])
	...
	par.update({
		...
		'n_exc'        		: int(par['n_total']*par['exc_inh_prop']),
	})

	par.update({
		'rg_exc': range(par['n_exc']),
		'rg_inh': range(par['n_exc'], par['n_total']),
		'w_in_mask': np.ones((par['n_input'], par['n_total']), dtype=np.float32),
		'w_rnn_mask': np.ones((par['n_total'], par['n_total']), dtype=np.float32) - np.eye(par['n_total'],dtype=np.float32),
		'w_out_mask': np.concatenate((np.ones((par['n_exc'], par['n_output']),dtype=np.float32),
									  np.zeros((par['n_total']-par['n_exc'], par['n_output']), dtype=np.float32)),
