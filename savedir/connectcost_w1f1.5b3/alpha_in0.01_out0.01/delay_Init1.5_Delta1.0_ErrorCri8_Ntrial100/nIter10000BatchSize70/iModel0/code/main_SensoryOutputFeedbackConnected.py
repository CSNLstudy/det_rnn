...
alpha_input                 = 0.01 # Chaudhuri et al., Neuron, 2015
alpha_output                = 0.01 # Chaudhuri et al., Neuron, 2015; Motor (F1) cortex's decay is in between input and hidden
...
def run_model(in_data, syn_x_init, syn_u_init):
    ...
    syn_x = syn_x_init
    syn_u = syn_u_init
    w_rnn = (1-tf.eye(par['n_total'])) * (par['EI_mask'] @ tf.nn.relu(var_dict['w_rnn']))
