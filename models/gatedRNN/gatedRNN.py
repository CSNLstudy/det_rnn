...
class gRNN(BaseModel):
    ...
    def evaluate(self, trial_info):
        ...
        [B, T, N, nullln] = outputs.shape #(has a null dimension for matrix multiplication in the rnncell)
        ...
        if self.hp['dropout'] > 0:
            ...
            out_dm = tf.nn.dropout(out,self.hp['dropout'], noise_shape = [1,1,N])
            out_em = tf.nn.dropout(out,self.hp['dropout'], noise_shape = [1,1,N])
        else:
            out_dm = out
            out_em = out
