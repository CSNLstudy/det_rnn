...
from det_rnn.base.functions import random_normal_abs, alternating, modular_mask, \
    w_rnn_mask

from models.schedule.scheduler import *
...
class mgRNN(BaseModel):
    def __init__(self, hp, dtype = tf.float32,
        ...
        super(mgRNN, self).__init__(hp,
                                   hypsearch_dict=hypsearch_dict,
                                   dtype=dtype,
                                   logger=logger)

        self._initialize_variable()
        print('(mgRNN) Network initialization complete')

    def _initialize_variable(self):
        print('(mgRNN) Building rnn...')
        ...
        self.rnn = tf.keras.layers.RNN(self.rnncell,
                                       ...
                                       name='RNN')
        self.dec = tf.keras.layers.Dense(
            self.hp['n_output_dm'] - self.hp['n_rule_output_dm'],
        ...
        if self.hp['gate_out']:
            self.dec_mdm = tf.keras.layers.Dense(
                self.hp['n_output_dm'] - self.hp['n_rule_output_dm'],
                kernel_regularizer=tf.keras.regularizers.l1_l2(
                    l1=self.hp['loss_L1'],
                    l2=self.hp['loss_L2']),
            ...
            self.dec_mda = tf.keras.layers.Dense(
                self.hp['n_output_dm'] - self.hp['n_rule_output_dm'],
                kernel_regularizer=tf.keras.regularizers.l1_l2(
                    l1=self.hp['loss_L1'],
                    l2=self.hp['loss_L2']),
        ...
        laplacmat = np.zeros(
            (self.hp['n_tuned_output'], self.hp['n_tuned_output']))
        laplacmat[0, 0] = -0.25
        laplacmat[1, 0] = 0.5
        laplacmat[2, 0] = -0.25
        ...
        self.laplacian = tf.constant(laplacmat, dtype=self.dtype,
                                     name='laplacian')
    ...
    def evaluate(self, trial_info):
        ...
        neural_input = tf.transpose(trial_info['neural_input'][:, :, :],
                                    perm=[1, 0, 2])
        if self.hp['stsp']:
        ...
        [B, T, N,
         nullln] = outputs.shape  # (has a null dimension for matrix multiplication in the rnncell)
        out = tf.squeeze(outputs, axis=-1)

        if self.hp['dropout'] > 0:
            # todo: this is used for training and testing. fix this.
            # drop different neurons for dm and em
        ...
        if self.hp['gate_out']:
            out_dm = (0 + self.dec_mdm(
                neural_input[:, :, :self.hp['n_rule_input']])) * self.dec(
                out_dm) + \
                     self.dec_mda(neural_input[:, :, :self.hp['n_rule_input']])

            out_em = (0 + self.est_mdm(
                neural_input[:, :, :self.hp['n_rule_input']])) * self.est(
                out_em) + \
    ...
    def calc_loss(self, trial_info, output):
        ...
        dec_desired_out = trial_info['desired_decision'][:, :,
                          self.hp['n_rule_output_dm']:]
        est_desired_out = trial_info['desired_estim'][:, :,
                          self.hp['n_rule_output_em']:]
        ...
        est_loss_mse = tf.reduce_mean(
            tf.gather(tf.math.square(est_desired_out - out_em),
                      self.hp['input_rule_rg']['estimation'], axis=0))
        dec_loss_mse = tf.reduce_mean(
            tf.gather(tf.math.square(dec_desired_out - out_dm),
        ...
        if self.hp['out_representation'] == 'probs':
            est_prob = out_em / tf.reduce_sum(out_em + EPSILON, axis=2,
                                              keepdims=True)
            dec_prob = out_dm / tf.reduce_sum(out_dm + EPSILON, axis=2,
                                              keepdims=True)
            ...
            # dec_prob = tf.nn.softmax(out_em, axis=2)
            # # average over time then batch
            # # log: cross entropy
            # est_loss_ce = tf.reduce_mean(
            #     tf.gather(-est_target,
        ...
        l1_loss = tf.reduce_mean(tf.abs(self.rnncell.rnnmat)) + tf.reduce_mean(
            tf.abs(self.rnncell.Win))
        l2_loss = tf.reduce_mean(
            tf.math.square(self.rnncell.rnnmat)) + tf.reduce_mean(
            tf.math.square(self.rnncell.Win))
        ...
        l1_loss += tf.reduce_sum(
            [tf.reduce_mean(tf.abs(v)) for v in self.dec.trainable_variables
             if v.name.find('bias') < 0])
        l1_loss += tf.reduce_sum(
            [tf.reduce_mean(tf.abs(v)) for v in self.est.trainable_variables
             if v.name.find('bias') < 0])
        l2_loss += tf.reduce_sum(
            [tf.reduce_mean(tf.square(v)) for v in self.dec.trainable_variables
             if v.name.find('bias') < 0])
        l2_loss += tf.reduce_sum(
            [tf.reduce_mean(tf.square(v)) for v in self.est.trainable_variables
             if v.name.find('bias') < 0])
        ...
        if self.hp['gate_out']:
            l1_loss += tf.reduce_sum([tf.reduce_mean(tf.abs(v)) for v in
                                      self.dec_mdm.trainable_variables
                                      if v.name.find('bias') < 0])
            l1_loss += tf.reduce_sum([tf.reduce_mean(tf.abs(v)) for v in
                                      self.est_mdm.trainable_variables
                                      if v.name.find('bias') < 0])
            l2_loss += tf.reduce_sum([tf.reduce_mean(tf.square(v)) for v in
                                      self.dec_mdm.trainable_variables
                                      if v.name.find('bias') < 0])
            l2_loss += tf.reduce_sum([tf.reduce_mean(tf.square(v)) for v in
                                      self.est_mdm.trainable_variables
                                      if v.name.find('bias') < 0])
            l1_loss += tf.reduce_sum([tf.reduce_mean(tf.abs(v)) for v in
                                      self.dec_mda.trainable_variables
                                      if v.name.find('bias') < 0])
            l1_loss += tf.reduce_sum([tf.reduce_mean(tf.abs(v)) for v in
                                      self.est_mda.trainable_variables
                                      if v.name.find('bias') < 0])
            l2_loss += tf.reduce_sum([tf.reduce_mean(tf.square(v)) for v in
                                      self.dec_mda.trainable_variables
                                      if v.name.find('bias') < 0])
...
class RNNCell(tf.keras.layers.Layer):
    def __init__(self, hp=None, **kwargs):
        if hp is not None:
            ...
            super(RNNCell, self).__init__(dtype=tf.float32,
                                          name='RNNCell')  # todo change dtype
    ...
    def build_rnn(self):
        ...
        self.add_loss(
            lambda: self.hp['loss_L1'] * tf.reduce_mean(tf.abs(self.rnnmat)))
        self.add_loss(
            lambda: self.hp['loss_L1'] * tf.reduce_mean(tf.abs(self.Win)))
        self.add_loss(lambda: self.hp['loss_L2'] * tf.reduce_mean(
            tf.math.square(self.rnnmat)))
        self.add_loss(lambda: self.hp['loss_L2'] * tf.reduce_mean(
            tf.math.square(self.Win)))

        self.add_loss(
            lambda: self.hp['loss_L1'] * tf.reduce_mean(tf.abs(self.Wmd_rnn_a)))
        self.add_loss(lambda: self.hp['loss_L2'] * tf.reduce_mean(
            tf.math.square(self.Wmd_rnn_a)))

        if self.hp['gate_rnn']:
            self.add_loss(lambda: self.hp['loss_L1'] * tf.reduce_mean(
                tf.abs(self.Wmd_rnn_x)))
            self.add_loss(lambda: self.hp['loss_L2'] * tf.reduce_mean(
                tf.math.square(self.Wmd_rnn_x)))

        # stsp parameters
        if self.hp['stsp'] is True:
            self.alpha_std_mat = tf.cast(
                alternating(self.hp['alpha_std'], self.hp['n_hidden']),
                self.dtype)
            self.alpha_stf_mat = tf.cast(
                alternating(self.hp['alpha_stf'], self.hp['n_hidden']),
                self.dtype)
            self.U_mat = tf.cast(alternating(self.hp['U'], self.hp['n_hidden']),
                                 self.dtype)

    def build_weights(self):
        # network weights
        self.rnnmat = tf.Variable(
            tf.random.normal([self.hp['n_hidden'], self.hp['n_hidden']]),
            name='rnnmat',
            dtype=self.dtype)
        self.Win = tf.Variable(
            tf.random.normal([self.hp['n_hidden'], self.hp['n_tuned_input']]),
            name='Win',
            dtype=self.dtype)

        self.Wmd_rnn_a = tf.Variable(
            tf.random.normal([self.hp['n_hidden'], self.hp['n_rule_input']]),
            name='Wmd_rnn_a', dtype=self.dtype)

        # gating weights
        if self.hp['gate_rnn']:
            self.Wmd_rnn_x = tf.Variable(tf.random.normal(
                [self.hp['n_hidden'], self.hp['n_rule_input']]),
                                         name='Wmd_rnn_x', dtype=self.dtype)

        self.EI_mask = tf.cast(w_rnn_mask(self.hp['n_hidden'],
                                          self.hp['exc_inh_prop']).transpose(),
                               self.dtype)  #
        # build EI mask for the recurrent weights
        if self.hp['tau_train']:
            self.tau_unnorm = tf.Variable(
                tf.random.normal([self.hp['n_hidden']]), name='tau_unnorm',
                dtype=self.dtype)
        else:
    ...
    def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
        states = [
            tf.zeros((batch_size, self.hp['n_hidden'], 1), dtype=self.dtype)]
        if self.hp['stsp']:
            states += [tf.ones((batch_size, self.hp['n_hidden'], 1))]
            states += [tf.tile(
                tf.expand_dims(tf.expand_dims(self.U_mat, axis=0), axis=-1),
                (batch_size, 1, 1))]
    ...
    def __call__(self, inputs, states, training=None):
        ...
        if self.hp['stsp']:
            ...
            alpha_std_mat = tf.expand_dims(
                tf.expand_dims(self.alpha_std_mat, axis=0), axis=-1)
            U_mat = tf.expand_dims(tf.expand_dims(self.U_mat, axis=0), axis=-1)
            alpha_stf_mat = tf.expand_dims(
                tf.expand_dims(self.alpha_stf_mat, axis=0), axis=-1)

            syn_x += alpha_std_mat * (1. - syn_x) - self.hp[
                'dt'] / 1000 * syn_u * syn_x * prev_h
            syn_u += alpha_stf_mat * (U_mat - syn_u) + self.hp[
                'dt'] / 1000 * U_mat * (1. - syn_u) * prev_h
        ...
        if self.hp['update'] == 'Masse':
            dh_pre = wrnn @ h_post + self.Win @ inputmat[:,
                                                self.hp['n_rule_input']:, :]
            dh_pre += self.Wmd_rnn_a @ inputmat[:, :self.hp['n_rule_input'], :]

            if self.hp['gate_rnn']:
                gate = (0 + self.Wmd_rnn_x @ inputmat[:,
                                             :self.hp['n_rule_input'],
                                             :])  # gated by rules
            else:
                gate = 1
            ...
            h = (1 - tf.expand_dims(alpha, axis=-1)) * prev_h + tf.expand_dims(
                alpha, axis=-1) * (gate * dh_pre)

        # inject noise
        if self.hp['rnn_noise_type'] is not None and dh_pre.shape[
            0] is not None:
            h = h + self.hp['noise_sd'] * \
                tf.random.normal(tf.shape(dh_pre), mean=0,
                                 stddev=tf.expand_dims(tf.sqrt(2 * alpha),
                                                       axis=-1),
                                 dtype=self.dtype)
    ...
    def build(self, hp, input_shape=None):
        ...
        if self.hp['stsp']:
            ...
            '''
            self.state_size = [self.hp['n_hidden'], self.hp['n_hidden'],
                               self.hp['n_hidden']]
        else:
            self.state_size = self.hp[
                'n_hidden']  # todo: add STSP states to the cell
        self.output_size = self.hp[
            'n_hidden']  # state_size and output_size is used by keras
