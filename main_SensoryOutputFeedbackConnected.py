...
def calc_loss(syn_x_init, syn_u_init, in_data, out_target, mask_train):
    ...
    MAP_dirs = tf.argmax(MAP_dirs, axis=1) - 1
    # MAP_dirs = MAP_dirs.numpy()
    ...
    est_error = tf.math.acos(tf.math.cos(2 * (MAP_dirs - Target_dirs) / 180 * np.pi)) / np.pi * 180 / 2
...
def append_model_performance(model_performance, loss, loss_orient, spike_loss, est_error, delay, itime, var_dict):
    ...
    model_performance['delay'].append()
...
for iModel in range(nModel[0], nModel[1]):
    ...
    model_performance = {'loss': [], 'loss_orient': [], 'spike_loss': [], 'est_error': [],
    ...
    for i in range(0, iteration):
        ...
        if i > 10:
            if np.mean(model_performance['est_error'][-par['N_conseq_epoch_est_error']:]) > par['criterion_est_error']:
                ...
                stimulus = Stimulus(par)
                print('##')
                print(str(cUpdate) + 'nd Delay Update from '
